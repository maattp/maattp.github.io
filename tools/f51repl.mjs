...
const PORT = 9224, URL = process.env.F51_URL || 'http://localhost:8000/apps/fable51kart/?dbg=1';
...
const chrome = spawn(CHROME, [`--remote-debugging-port=${PORT}`, '--headless=new', '--use-gl=angle', '--use-angle=swiftshader', '--enable-unsafe-swiftshader',
  '--autoplay-policy=no-user-gesture-required', '--disable-background-timer-throttling', '--window-size=1280,720', '--no-first-run', '--user-data-dir=/tmp/f51-repl-profile', 'about:blank'], { stdio: 'ignore' });
