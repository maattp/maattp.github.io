...
const chrome = spawn(CHROME, [`--remote-debugging-port=${PORT}`, '--headless=new', '--no-first-run', '--user-data-dir=/tmp/sf-icon-profile', 'about:blank'], { stdio: 'ignore' });
try {
  let page;
  ...
  await send('Page.navigate', { url: 'http://localhost:8000/apps/spaceflight/gen-icon.html' });
  ...
  for (const [name, key] of [['icon-512', 512], ['icon-192', 192], ['icon-180', 180]]) {
    ...
    writeFileSync(`apps/spaceflight/${name}.png`, Buffer.from(url.split(',')[1], 'base64'));
    console.log('wrote apps/spaceflight/' + name + '.png');
  }
} finally { chrome.kill(); }
