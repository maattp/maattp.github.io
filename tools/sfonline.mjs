// Two-browser online smoke test for apps/spaceflight: host creates a room,
...
const OUT = 'tools/data/sfshots/online'; mkdirSync(OUT, { recursive: true });
...
async function browser(port, tag) {
  const proc = spawn(CHROME, [`--remote-debugging-port=${port}`, '--headless=new', '--use-gl=angle', '--use-angle=swiftshader', '--enable-unsafe-swiftshader',
    '--autoplay-policy=no-user-gesture-required', '--disable-background-timer-throttling', '--window-size=480,270', '--no-first-run', `--user-data-dir=/tmp/sf-online-${tag}`, 'about:blank'], { stdio: 'ignore' });
...
const A = await browser(9251, 'host'), B = await browser(9252, 'client');
try {
  await A.ev("document.getElementById('nameInput').value = 'Hosty'; document.getElementById('createRoomBtn').click();");
  let code = '';
  for (let i = 0; i < 40 && !code; i++) { await sleep(250); code = await A.ev('return __sf.net.code'); }
  ...
  for (let i = 0; i < 40; i++) { await sleep(250); if (await B.ev('return __sf.net.mode') === 'client') break; }
  check(await B.ev('return __sf.net.mode') === 'client', 'client joined as client');
  await sleep(600);
  check(await A.ev('return __sf.net.players.length') === 2, 'host sees 2 players');
  ...
  await A.ev('__sf.setLapsNet(1);');
  await A.ev("document.getElementById('lobbyStartBtn').click();");
  for (let i = 0; i < 60; i++) { await sleep(250); if (await B.ev("return __sf.state === 'racing'") && await A.ev("return __sf.state === 'racing'")) break; }
  check(await A.ev("return __sf.state") === 'racing' && await B.ev("return __sf.state") === 'racing', 'both racing after countdown');
  const auto = "const p = __sf.player; p.ai = { lane: 0, skill: 0.95, aggr: 0.6, phase: 0, jitter: 0 }; p._auto = true; return p.pid;";
  const pidA = await A.ev(auto), pidB = await B.ev(auto);
  console.log('   pids', pidA, pidB, 'A mode', await A.ev('return __sf.net.mode'), 'B mode', await B.ev('return __sf.net.mode'));
  await sleep(8000);
  const snapTick = await B.ev('return __sf.net.lastSnapTick');
  check(snapTick > 0, 'client receiving snapshots (tick ' + snapTick + ')');
  check(await A.ev(`return !!__sf.net.remoteCtrl[${pidB}]`), 'host receiving client inputs');
  const posB = await B.ev('const p = __sf.player; return [p.pos.x, p.pos.y, p.pos.z, p.speed]');
  const posBonA = await A.ev(`const k = __sf.machines.find((k) => k.pid === ${pidB}); return [k.pos.x, k.pos.y, k.pos.z, k.speed, k.kind]`);
  ...
  const posA = await A.ev('const p = __sf.player; return [p.pos.x, p.pos.z]');
  const posAonB = await B.ev(`const k = __sf.machines.find((k) => k.pid === ${pidA}); return [k.pos.x, k.pos.z, k.kind]`);
  ...
  console.log('   rtc host→client:', await A.ev(`const r = __sf.net.rtc[${pidB}]; return r ? [r.pc.connectionState, r.pc.iceConnectionState, r.pc.iceGatheringState, r.open] : 'none'`), '| client:', await B.ev(`const r = __sf.net.rtc[${pidA}]; return r ? [r.pc.connectionState, r.pc.iceConnectionState, r.open] : 'none'`));
  ...
  for (let i = 0; i < 150 && !done; i++) {
    await sleep(2500);
    done = await B.ev("return __sf.player.finished") && await A.ev("return __sf.player.finished");
    if (i % 12 === 11) console.log('   t+' + ((i + 1) * 2.5) + 's host clock', await A.ev('return [__sf.raceClock.toFixed(0), __sf.player.laps, (__sf.player.s/__sf.TRACK.len).toFixed(2), __sf.player.finished, __sf.machines.filter(k=>k.finished).length]'), 'client', await B.ev('return [__sf.raceClock.toFixed(0), __sf.player.laps, (__sf.player.s/__sf.TRACK.len).toFixed(2), __sf.player.finished, __sf.state, __sf.player.energy.toFixed(0)]'));
  }
  check(done, 'both players finished the lap (host-confirmed on the client)');
  await sleep(3000);
  await A.shot('results'); await B.shot('results');
  check(await B.ev("return document.getElementById('resultScreen').classList.contains('on')"), 'client shows results');
  const rankB = await B.ev('return __sf.player.rank'), rankBonA = await A.ev(`return __sf.machines.find((k) => k.pid === ${pidB}).rank`);
  ...
  await sleep(1500);
  check(await B.ev("return document.getElementById('lobbyScreen').classList.contains('on') && __sf.state === 'menu'"), 'client back in lobby after host rematch');
