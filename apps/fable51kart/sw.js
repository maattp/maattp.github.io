// Fable51 Kart service worker: solo play fully offline. Shell-only, fast
// install (iOS standalone aborts slow installs on quick relaunches); the
// pinned Three.js r128 CDN build is cached lazily and copied forward across
// CACHE bumps. Same-origin shell is stale-while-revalidate, revalidated by
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    const olds = keys.filter((k) => k.startsWith('fable51kart-') && k !== CACHE);
